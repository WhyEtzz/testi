...
case prefix+'stext':
 if(isReg(obj)) return
            if(cekumur(cekage)) return
 if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
  const kats = body.slice(7)
 tobz.sendStickerfromUrl(from, `https://textoverimage.moesif.com/image?image_url=https%3A%2F%2Fcpng.pikpng.com%2Fpngl%2Fs%2F54-544333_white-blur-png-white-blur-rectangle-png-clipart.png&overlay_color=22194d00&text=${kats}&text_color=ffffffff&text_size=64&margin=2&y_align=middle&x_align=center`)
break
        case prefix+'checkip':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
